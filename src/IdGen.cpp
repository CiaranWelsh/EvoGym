...
namespace evo {
    ...
    IdGen::IdGen(std::string base, long start_number, int num_digits) :
            base_(std::move(base)),
    ...
    bool IdGen::operator==(const IdGen &rhs) const {
    ...
    int IdGen::countDigits(long long int n) {
        if (n == 0) {
            return 1;
        }
        int count = 0;
    ...
    std::string IdGen::generate() const {
        return generate(start_number_);
    }

    std::string IdGen::generate(long n) const {
